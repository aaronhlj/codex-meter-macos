...
应用不会读取 `~/.codex/sessions` 或 `~/.codex/auth.json`，也不会自行上传提示词、会话内容或账户凭据。详见 [隐私说明](PRIVACY.md)。
...
The app does not read `~/.codex/sessions` or `~/.codex/auth.json`, or independently upload prompts, conversations, or account credentials. See [Privacy](PRIVACY.md) for details.
